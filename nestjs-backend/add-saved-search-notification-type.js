// Genişlet notifications.type ENUM kolonunu — saved_search_match değerini ekle.
...
(async () => {
  ...
  console.log('[fix] applying ALTER MODIFY (18 enum values)');
  await c.query(ddl);

  // saved_job_searches.lastNotifiedAt
  try {
    await c.query("ALTER TABLE `saved_job_searches` ADD COLUMN `lastNotifiedAt` DATETIME NULL");
    console.log('[fix] added saved_job_searches.lastNotifiedAt');
  } catch (e) {
    if (e.code === 'ER_DUP_FIELDNAME') console.log('[fix] saved_job_searches.lastNotifiedAt already exists');
    else throw e;
  }
