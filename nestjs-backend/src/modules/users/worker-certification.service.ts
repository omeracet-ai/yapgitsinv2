...
export class WorkerCertificationService {
  ...
  async create(
    ...
    if (!name || name.length > 200)
      throw new BadRequestException('name gerekli (max 200)');
    if (!issuer || issuer.length > 200)
      throw new BadRequestException('issuer gerekli (max 200)');
    const issued = new Date(dto.issuedAt);
    if (isNaN(issued.getTime()))
      throw new BadRequestException('issuedAt geçersiz');
  ...
  async deleteOwn(userId: string, certId: string): Promise<{ ok: true }> {
    ...
    if (cert.userId !== userId)
      throw new ForbiddenException('Bu sertifika size ait değil');
