...
export class SavedSearchAlertService {
  ...
  async runAlerts(): Promise<void> {
    ...
    for (const search of searches) {
      const since =
        search.lastNotifiedAt ?? new Date(now.getTime() - 60 * 60 * 1000);
      ...
      if (typeof c.budgetMin === 'number') {
        qb.andWhere('(job.budgetMax >= :bMin OR job.budgetMax IS NULL)', {
          bMin: c.budgetMin,
        });
      }
      if (typeof c.budgetMax === 'number') {
        qb.andWhere('(job.budgetMin <= :bMax OR job.budgetMin IS NULL)', {
          bMax: c.budgetMax,
        });
      }
