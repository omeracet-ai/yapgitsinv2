...
interface CoordState { lat: string; lng: string; }

// Haritada hangi katmanlar görünsün
interface MapDisplay { jobs: boolean; users: boolean; }

export default function HaritaPage() {
  const [tab, setTab] = useState<Tab>("jobs");
  const [mapDisplay, setMapDisplay] = useState<MapDisplay>({ jobs: true, users: false });
  ...
  useEffect(() => {
    if (tab === "users" && !usersLoaded) {
      ...
      api.getUsers({ limit: 100, page: 1 })
        .then((r) => { setUsers(r.items); setUsersLoaded(true); })
        .catch((e) => setUsersError((e as Error).message))
        .finally(() => setUsersLoading(false));
    }
  }, [tab, usersLoaded]);

  // Kullanıcılar katmanı açılınca veriyi yükle
  useEffect(() => {
    if (mapDisplay.users && !usersLoaded && !usersLoading) {
      setUsersLoading(true);
      api.getUsers({ limit: 100, page: 1 })
        .then((r) => { setUsers(r.items); setUsersLoaded(true); })
        .catch((e) => setUsersError((e as Error).message))
  ...
  }, [mapDisplay.users, usersLoaded, usersLoading]);
  ...
  const jobMapItems: MapItem[] = jobs.map((j) => ({
    id: j.id,
  ...
  // Hangi katmanın primary, hangisinin overlay olduğu tab'a göre belirlenir
  const primaryItems  = tab === "jobs"
    ? (mapDisplay.jobs  ? jobMapItems  : [])
    : (mapDisplay.users ? userMapItems : []);
  const overlayItems  = tab === "jobs"
    ? (mapDisplay.users ? userMapItems : [])
    : (mapDisplay.jobs  ? jobMapItems  : []);
  const overlayLabel  = tab === "jobs" ? "Usta" : "İlan";

  const selectedItem = editItems.find((i) => i.id === selectedId) ?? null;
  ...
  const handleMarkerClick = useCallback((id: string) => {
    // Overlay marker'a tıklanmışsa tab'ı değiştir
    const inEdit = editItems.some((i) => i.id === id);
    if (!inEdit) {
      setTab((t) => t === "jobs" ? "users" : "jobs");
    }
    selectItem(id);
  ...
  async function handleSave() {
    ...
    if (isNaN(lat) || isNaN(lng)) { setSaveMsg("Geçersiz koordinat."); return; }
    setSaving(true);
    setSaveMsg(null);
    try {
      if (tab === "jobs") {
        await api.setJobLocation(selectedId, lat, lng);
        setJobs((prev) => prev.map((j) => j.id === selectedId ? { ...j, latitude: lat, longitude: lng } : j));
      } else {
        await api.setUserLocation(selectedId, lat, lng);
        setUsers((prev) => prev.map((u) => u.id === selectedId ? { ...u, latitude: lat, longitude: lng } : u));
      }
  ...
  const missingCount = editItems.filter((i) => i.latitude == null).length;
  ...
  return (
    <div className="flex flex-col h-full max-h-[calc(100vh-3.5rem-3rem)]">
      ...
      {/* Düzenleme Sekmeleri + Harita Görüntüleme Kontrolü */}
      <div className="mb-4 flex items-center justify-between flex-wrap gap-3">
        {/* Sol: düzenleme tab */}
        <div className="flex gap-2 items-center">
          <span className="text-xs text-gray-400 font-medium">Düzenle:</span>
        ...
        {/* Sağ: harita görüntüleme kontrolü */}
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400 font-medium">Haritada Göster:</span>
          <label className="flex items-center gap-1.5 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={mapDisplay.jobs}
              onChange={(e) => setMapDisplay((d) => ({ ...d, jobs: e.target.checked }))}
          ...
          <label className="flex items-center gap-1.5 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={mapDisplay.users}
              onChange={(e) => setMapDisplay((d) => ({ ...d, users: e.target.checked }))}
      ...
      <div className="flex flex-1 gap-4 min-h-0">
        ...
        <div className="w-72 shrink-0 flex flex-col border border-gray-200 rounded-xl bg-white overflow-hidden">
          <div className="px-3 py-2 border-b border-gray-100 text-xs text-gray-400 font-medium uppercase tracking-wide">
            {tab === "jobs" ? "İlanlar" : "Ustalar"} ({editItems.length})
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            {isLoading ? (
              <div className="py-8 text-center text-sm text-gray-400 animate-pulse">Yükleniyor…</div>
            ) : editItems.length === 0 ? (
          ...
          {selectedItem ? (
            <div className="bg-white border border-gray-200 rounded-xl px-4 py-3 flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <input
                  type="number" step="any" value={coords.lng}
                ...
                {saveMsg && (
                  <span className={`text-xs font-medium ${saveMsg.startsWith("Hata") ? "text-red-600" : "text-green-600"}`}>
                    {saveMsg}
                  </span>
                )}
              </div>
