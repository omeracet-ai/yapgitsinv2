#!/usr/bin/env node
/**
 * Honest city-centroid coordinate backfill for jobs/users (SQLite).
 *
 * Replaces the prior jitter-based fake-coords backfill.
 * - Only writes verified TR city centroids (no random offset).
 * - Marks every row written as approximate via location_approx=1 + location_source='city_centroid'.
 * - Idempotent schema migration (adds location_approx / location_source if missing).
 *
 * Usage:
 *   node scripts/backfill-coordinates.js              # dry-run (default)
 *   node scripts/backfill-coordinates.js --dry-run    # explicit dry-run
 *   node scripts/backfill-coordinates.js --apply      # write changes
 *
 * Targets:
 *   jobs.location  -> jobs.latitude/longitude     (city parsed from `location`)
 *   users.city     -> users.latitude/longitude    (city from `city` column)
 *
 * Only rows where (latitude IS NULL OR longitude IS NULL OR (latitude=0 AND longitude=0))
 * are considered. Unknown cities are reported, not guessed.
 */
...
// Common aliases / shorthands -> canonical key
const ALIASES = {
...
// Normalize TR -> ASCII lowercase
function normalizeCity(raw) {
  if (!raw || typeof raw !== 'string') return null;
  // First token before comma/slash (e.g. "İstanbul, Kadıköy" -> "İstanbul")
...
(async () => {
  console.log(`[backfill-coords] DB: ${dbPath}`);
