...
### Yapgitsin tab → Hizmet İlanları → "Teklif Al" — 2026-05-19 23:12
- 🐛 **BUG #4 — Müşteri yanlış rol arayüzüne düşüyordu**: Offer-tipi ilan kartındaki "Teklif Al" butonu (`service_listings_screen.dart:486`) `JobDetailScreen` açıyordu, o ekran ustaların "Teklif Ver" formunu gösterdiği için müşteri kendi rolünden çıkıp usta rolüne sıçramış oluyordu.
- ✅ **Fix uygulandı** ([2e2bd090](.)): Button artık `/profil/${job.customerId}` (Phase Two-Sided'da offer'larda `customerId` = ilanı yayınlayan ustanın id'si) açıyor. Müşteri ustanın public profile'ından "Randevu Al" CTA ile `/randevu-olustur/{ustaId}` akışına geçip teklif/randevu talep ediyor. İkon `send_rounded` → `handshake_rounded`.
